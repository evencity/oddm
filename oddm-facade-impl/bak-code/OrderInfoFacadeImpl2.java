...
import com.apical.oddm.facade.constant.CrudConst;
import com.apical.oddm.facade.constant.OrderInfoConst;
...
public class CopyOfOrderInfoFacadeImpl implements OrderInfoFacade{

	private static final Logger log = LoggerFactory.getLogger(CopyOfOrderInfoFacadeImpl.class);
	...
	public void edit(OrderInfoCommand orderInfoCommand) {
		...
		int i;
		//变更记录计算
	/*	if (orderInfo.getState() >= OrderInfoConst.approved) {//审核通过
			OrderInfo orderInfoBefore = new OrderInfo();
			BeanUtils.copyProperties(orderInfo, orderInfoBefore);
		...
		if (orderInfoCommand.getOrderHardwareCommands() != null  && orderInfoCommand.getOrderHardwareCommands().size() > 0){
			Set<OrderHardwareCommand> orderHardwareCommands = orderInfoCommand.getOrderHardwareCommands();
			Set<OrderHardware> orderHardwares = orderInfo.getOrderHardwares();//new HashSet<OrderHardware>();
			for(OrderHardwareCommand orderHardwareCommand : orderHardwareCommands){
				OrderHardware orderHardware = new OrderHardware();
		...
		//	Set<OrderHardware> orderHardwares2 = orderInfo.getOrderHardwares();
			orderInfo.setOrderHardwares(orderHardwares);
		}
	...
	private void addEditRecord(OrderInfo orderInfoBefore, OrderInfoCommand orderInfoCommand) {
		...
		ThreadPoolBiz.getInstance().execute(new Runnable(){
			@Override
			public void run() {
				OrderInfoAlt alt = new OrderInfoAlt();
				alt.setOrderInfo(new OrderInfo(orderInfoBefore.getId()));//外键
				alt.setOperator("张珊");
				try {
					if (orderInfoBefore != null && orderInfoCommand != null){
						//判断订单主体
						if (StringUtils.isBlank(orderInfoBefore.getOrderNo())) {
							if (StringUtils.isNotBlank(orderInfoCommand.getOrderNo())) {
								alt.setBeforeInfo(null);
								alt.setAfterInfo(orderInfoCommand.getOrderNo());
