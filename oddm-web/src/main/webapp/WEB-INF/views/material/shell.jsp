...
<script type="text/javascript">
	var dataGrid;
	var materialBare_shellIds = "${shellIds}";
	$(function() {
		 if(materialBare_shellIds != ""){
				var shellIds = materialBare_shellIds.split(',');
				for(var i in shellIds){
					var shell = shellIds[i].split('|');
					var content = '<li id='+ shell[0] +'>'+ shell[1] +'</li>'
					$('#choose_shell').append(content);
				}
				
		}else{
			$.ajax({
				url:'${ctx}' + '/materialBare/listforShell',
				type:"GET",
				dataType: "json",
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/materialBare/dataGridforShell',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : false,
			...
			onDblClickRow : function(index, row){
				if($('#choose_shell li').length > 0){
					var falg = false;
					$('#choose_shell li').each(function(){
						var id = $(this).attr('id');
						if(id == row.id){
							falg = true;
						}   
		...
		$('#choose_shell').off('dblclick','li').on('dblclick','li',function(){
			$(this).remove();
		});
		
		$('#shell_materialName').off('keydown').on('keydown',function(event){
			 ...
			 if (event.keyCode == 13) {//keyCode=13是回车键
				/*  console.log(event.keyCode)
				 ...
				 dataGrid.datagrid('load', {
						name:$('#shell_materialName').val().trim(),
					});
             }
		});
	});
			...
			<div data-options="region:'north',border:false" title="" style="overflow: auto;padding: 3px;" >
				<!-- <form id="searchForm"> -->
					<table>
						<tr>
							<th>物料名称:</th>
							<td><input name="name" placeholder="请输入物料名称" id="shell_materialName"/></td>
							<td>
