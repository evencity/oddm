...
<script type="text/javascript">
	var dataGrid;
	var materialBare_hardwareIds = "${hardwareIds}";
	$(function() {
		 if(materialBare_hardwareIds != ""){
			var hardwareIds = materialBare_hardwareIds.split(',');
			for(var i in hardwareIds){
				var hardware = hardwareIds[i].split('|');
				var content = '<li id='+ hardware[0] +'>'+ hardware[1] +'</li>'
				$('#choose_hardware').append(content);
			}
			
		} 
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/materialBare/dataGridforBare?type=1',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : false,
			...
			onDblClickRow : function(index, row){
				if($('#choose_hardware li').length > 0){
					var falg = false;
					$('#choose_hardware li').each(function(){
						var id = $(this).attr('id');
						if(id == row.id){
							falg = true;
						}   
		...
		$('#choose_hardware').off('dblclick','li').on('dblclick','li',function(){
			$(this).remove();
		});
	});
	function searchFun() {
...
<div class="easyui-layout" data-options="fit:true,border:false" >
	<div data-options="region:'west',border:false" title="" style="overflow:auto;padding: 3px;width:30%;" >
		<div class="choose">
			...
			<div ><ul id="choose_hardware"></ul></div>
		</div>
