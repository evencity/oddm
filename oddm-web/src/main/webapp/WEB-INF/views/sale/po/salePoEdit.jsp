 ...
 //提交到后台
	$(function() {
		...
		if (jsonCurrency != null && ""!=jsonCurrency && typeof(jsonCurrency) != "undefined") {
			...
			dataCurr.push({value: "USD"}); dataCurr.push({value: "RMB"});
		}
		...
		$("#addImage").click(function(){
			...
			var del = "<img class=\"delImage\" onclick=\"javascript: deleteEvent(this);\" src=\"${ctx}/jslib/easyui1.4.2/themes/icons/del.png\" style=\"cursor:pointer;\"></img>";
			...
			var td5 = "<td class=\"td_currency\" colspan=\"1\" style=\"width: 5%;text-align:center;color: blue;\">"+currency+"</td>";
	 	...
	 	function reloadCss() {
			...
			//审核通过后，以前录入的产品名字就不能修改
			/* $('.productName2').attr("readonly",true);
			$('.productName2').css({
		...
		$('.tr_insert_new').each(function(){
	        ...
	        var qlity = tdArr.eq(3).find('input').val(); //产品数量
	        var uniP = tdArr.eq(5).find('input').val(); //单价
	...
	function deleteEvent(object) {
	    ...
	    var unitPrice = 0;
	    var description;
		$(current).parent().parent().each(function(){ //判断任意一项不为空，则提示，避免误删
	        tdArr = $(this).children();
	        number = tdArr.eq(0).find('input').val()==null?null:tdArr.eq(0).find('input').val().trim(); //产品编号
	        productName = tdArr.eq(1).find('input').val()==null?null:tdArr.eq(1).find('input').val().trim(); //产品类型
	        specification = tdArr.eq(2).find('textarea').val()==null?null:tdArr.eq(2).find('textarea').val().trim(); //规格型号
	        quality = tdArr.eq(3).find('input').val(); //产品数量
	        unitPrice = tdArr.eq(5).find('input').val(); //单价
	        description = tdArr.eq(7).find('textarea').val()==null?null:tdArr.eq(7).find('textarea').val().trim(); //收入金额
		}); 
		if ((number != "" && number != null)
			...
			$.messager.confirm('确认对话框', '当前行不为空，是否继续删除？', function(handle){
				if (handle){//确定删除
...
<div class="styleeasyui-layout" data-options="fit:true,border:false" >
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;padding: 3px;width: 95%;margin:0 3% 0 2%" >
		<form id="salePoEditForm" method="post">
			<table class="tableGrid" style="border-collapse:collapse; border:solid 1px gray;width:100%;background-color: #f0f4f7"> <!-- ;margin-left: 6px -->
				...
				<tr>
		            <td colspan="26" valign="bottom" style="text-align:center;text-align:center;">电话:<input id="ip_tel" name="tel" style="background-color:inherit ;border:0px;height: 100%" type="text" placeholder="请输入" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']" value="${salePoDto.tel }"> 
		                        传真:<input id="ip_fax" name="fax" style="height: 100%;background-color:inherit ;border:0px;" type="text" placeholder="请输入" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']" value="${salePoDto.fax }"> 网址:<input id="ip_homepage" name="homepage" style="height: 100%;background-color:inherit ;border:0px;" type="text" placeholder="请输入" class="easyui-validatebox" data-options="required:true,validType:['length[0,100]']" value="${salePoDto.homepage }"></td>
		        </tr>
				<tr>
		            <td colspan="26" valign="center" style="font-weight: bold;font-size:22px;height: 40px;text-align:center;">Purchase Order</td>
				...
				<tr class="tr_border" style="height: 35px;">
					...
					<td colspan="5" style="width: 15%;"><input id="ip_piNo" name="piNo" style="height: 100%;width: 100%;text-align: center" type="text" placeholder="请输入" class="easyui-validatebox" data-options="required:false,validType:['length[0,64]']" value="${salePoDto.piNo }"></td>
				</tr>
				 <tr class="tr_border" style="height: 35px;">
				...
				<tr style="height: 35px;">
					<td colspan="5" style="width: 26.5%;">产品经理：<input id="ip_pm" name="pm" style="height: 100%" type="text" placeholder="输入产品经理" class="easyui-validatebox" data-options="validType:['length[0,64]']" value="${salePoDto.pm }"></td>
				...
				<tr class="tr_border" style="height: 35px;">
					...
					<td colspan="10" style="width: 51%;text-align:center;"><input id="ip_dateExaminePre" name="dateExaminePre" style="width: 30%;height: 100%" type="text" class="easyui-datebox" data-options="required:false,editable:false " value="${salePoDto.dateExaminePre }"></td>
				...
				<c:forEach items="${salePoDto.salePoListsDto }" var="item" varStatus="status">  
				  <tr id="${item.id}" class="tr_insert tr_insert_new" style="height: 80px;">  
				    <td colspan="3" style="width: 12%;"><input class="number" name="salePoListsDto.number" style="width: 100%;height: 100%;text-align:center;" placeholder="请输入" type="text" class="easyui-validatebox" data-options="validType:['length[0,64]']" value="${item.number}"></td>
				    <c:choose>
					   <c:when test="${salePoDto.state >= '4'}">  
				    		<td colspan="3" style="width: 12%;"><input class="productName productName2" name="salePoListsDto.productName" style="width: 100%;height: 100%;text-align:center;" placeholder="请输入" type="text" class="easyui-validatebox" data-options="editable:false,validType:['length[0,64]']" value="${item.productName}"></td>
					   </c:when>
					   <c:otherwise>
					   		<td colspan="3" style="width: 12%;"><input class="productName" name="salePoListsDto.productName" style="width: 100%;height: 100%;text-align:center;" placeholder="请输入" type="text" class="easyui-validatebox" data-options="validType:['length[0,64]']" value="${item.productName}"></td>
					   </c:otherwise>
					</c:choose>
				    <td colspan="7" style="width: 30%;"><textarea name="salePoListsDto.specification" style="width: 100%;height: 100%;word-break:break-all;" placeholder="请输入" class="easyui-validatebox" data-options="validType:['length[0,200]']" >${item.specification }</textarea></td>
				    <td colspan="1" style="width: 8%;"><input class="quality" name="salePoListsDto.quality" style="width: inherit;height: inherit" placeholder="请输入" type="text" class="easyui-numberbox" value="${item.quality }"></td>
					<td class="td_currency" colspan="1" style="width: 5%;text-align:center;color: blue;">${salePoDto.currency }</td>
					<td colspan="1" style="width: 8%;"><input class="unitPrice" name="salePoListsDto.unitPrice" style="width: inherit;height: inherit" placeholder="请输入" type="text" class="easyui-numberbox" value="${item.unitPrice }"></td>
					<td class="td_prices" colspan="5" style="width: 10%;color: blue;">0.00</td>
					<td colspan="4" style="width: 12.5%;" valign="middle" ><textarea name="salePoListsDto.description" style="width: 100%;height: 100%;; font-weight: bold;" placeholder="请输入" class="easyui-validatebox" data-options="validType:['length[0,100]']" >${item.description }</textarea></td>
					<td colspan="1" style="width: 1.5%;"><img class="delImage" src="${ctx}/jslib/easyui1.4.2/themes/icons/del.png" style="cursor:pointer;"></img></td>
				  </tr>  
				</c:forEach>  
				...
				<tr class="tr_border" style="height: 35px;">  
					...
					<td colspan="5" style="width: 15%;" class="t_title">币种:<input id="ip_currency" name="currency" style="width: 45%;height: 100%;color:red" type="text" placeholder="输入币种" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']" value="${salePoDto.currency }"></td>				</tr>
				<tr>
					<td colspan="26" style="height: 200px;"><textarea id="tx_description" name="description" style="width: 100%;height: 100%;" placeholder="请输入" maxlength="2000" >${salePoDto.description }</textarea></td>
				</tr>
				<tr class="tr_border" style="height: 35px;">
				...
				<tr class="tr_border" style="height: 40px;">
					<td class="t_title" colspan="6" style="width: 24%;"><input id="ip_maker" name="maker" style="width: 100%;height: 100%;text-align:center;" placeholder="请输入" type="text" class="easyui-validatebox" data-options="validType:['length[0,64]']" value="${salePoDto.maker }"></td>
					<td colspan="5" style="width: 25.5%;"></td>
			...
			<br>
			<div style="text-align: center;font-weight: bold;"><a href="javascript:void(0)" id="btn-summit" class="easyui-linkbutton" data-options="iconCls:'icon-ok'">保存修改</a></div>
		</form>
	</div>
</div>
</body>
...
<script>
function summit() {
	var isValid = $('#salePoEditForm').form('validate'); // 验证不通过则不执行
	if (!isValid) {
		return; // 验证不通过则不执行
	}
	...
	$('.tr_insert_new').each(function(){
 	...
 	var salePoCmd = {
			...
			approver: "${salePoDto.approver }",
			approverId: "${salePoDto.approverId }",
			salePoListCmd: salePoListCmd,
	};
	$.ajax({
		url : '${pageContext.request.contextPath}/salePo/edit',
		type : "POST",
