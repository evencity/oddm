...
<script type="text/javascript">	
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					$.ajax({
						type: 'POST',
					    url: "${ctx}/salePo/checkOrderForOrder?orderNo="+orderNo, 
					    dataType: "json",
	...
	function excelFun() {
		var row = $('#dataGrid').datagrid('getSelected');
		if (row){
			//console.log('Item ID:'+row.id+"piNo:"+row.piNo);
			download_file("${ctx}/salePo/excelExport?id="+row.id);
		} else {
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/salePo/addAll')}">
			<a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
		</c:if>
		<c:if test="${fn:contains(sessionInfo.resourceList, '/salePo/editAll')}">
			<a onclick="editFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-edit'">编辑</a>
		</c:if> 
		<c:if test="${fn:contains(sessionInfo.resourceList, '/salePo/deleteAll')}">
			<a onclick="deleteFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-del'">删除</a>
		</c:if> 
		 <c:if test="${fn:contains(sessionInfo.resourceList, '/salePo/excelAll')}">
