	    ...
	    <table border="0" cellpadding="0" cellspacing="14px" class="ordertable">
    	  <caption  class="tabletitle">LC收款台账明细：</caption >
     	  <tr>
	            ...
	            <td class=""> <input id="creditNo" value="" class="easyui-validatebox" data-options="required:false,validType:['piNoValidate']"/></td>
	            <td class="lefttd" width="15%">信用证总金额：</td>
	            <td class=""> <input id="creditAmount" value="0.00"/></td>
	        ...
	        <tr>
	            ...
	            <td class=""> <input id="invoiceNo" value="" class="easyui-validatebox" data-options="required:true,validType:['piNoValidate']"/></td>
	            <td class="lefttd" width="15%">发票金额：</td>
	            <td class="" > <input id="invoiceAmount" value="0.00"/></td>
	        	<td class="lefttd" width="10%">币种：</td>
	            <td class=""><input id="currency" value="USD" name="currency" style="width: 52%;color:red" type="text" placeholder="输入币种" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']"/></td>
	        	<td class="lefttd" width="10%"></td>
	            <td class=""></td>
	        </tr>
	        <tr>
	            <td class="lefttd" width="10%">客户：</td>
	            <td class=""> <input id="clientName" value="" class="easyui-validatebox" data-options="required:false,validType:['length[0,64]']"/></td>
           	    <td class="lefttd" width="15%">即期/远期：</td>
	            <td class=""><input id="spotForward" value="" class="easyui-validatebox" data-options="required:false,validType:['length[0,64]']"/></td>
           	    <td class="lefttd" width="10%">交单日期：</td>
	            <td class=""><input id="dateDelivery" value="" class="easyui-datebox"  data-options="editable:false,required:true" style="height: 30px"/></td> 
	         	<td class="lefttd" width="10%">到账日期：</td>
	            <td class=""><input id="dateGet" value="" class="easyui-datebox"  data-options="editable:false,required:false" style="height: 30px"/></td> 
	        </tr>
	        <tr>
	         	<td class="lefttd" width="15%">订单号：</td>
	            <td class="" colspan="3"><textarea id="orderNo" class="textareacss easyui-validatebox" data-options="required:false,validType:['orderNoValidate','length[0,200]']" rows="3" onkeydown="checkEnter(event)"></textarea></td>
...
function summit() {
	var isValid = $('#saleLcAddForm').form('validate'); // 验证不通过则不执行
	if (!isValid) {
		return; // 验证不通过则不执行
	}
