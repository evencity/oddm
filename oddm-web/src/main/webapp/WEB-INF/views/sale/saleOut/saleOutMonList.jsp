...
<style type="text/css">
	...
	input {border:1px solid #CCC;font-size:12px;border-radius: 4px;}//圆角
</style>
<script type="text/javascript">	
	var dataGrid;
	var db;
	var orderNo='${saleOutDto.orderNo}';
	var clientName='${saleOutDto.clientName}';
	var seller='${saleOutDto.seller}';
	var shipmentMonth='${saleOutDto.shipmentMonth}';
	if(shipmentMonth == null || shipmentMonth == "") {
		var defaultDate = new Date();
		shipmentMonth = defaultDate.getFullYear() + '-' + (defaultDate.getMonth() + 1);
	}
	...
	$(function() {
		...
		db.datebox('setValue', shipmentMonth); 
		...
		//加载dataGrid数据
		dataGridFun(orderNo, clientName, seller, shipmentMonth);
	...
	function dataGridFun(orderNo, clientName, seller, shipmentMonth) {
		//如果月份不为空，值显示统计页面，会以周分组
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/saleOut/dataGrid?orderNo='+orderNo+'&clientName='+clientName+'&seller='+seller+'&shipmentMonth='+shipmentMonth,
			striped : true,
	        ...
	        emptyMsg: '没有找到相关数据',
			pageSize : 2000,
			pageList : [10, 20, 30, 40, 50, 100, 2000],
			frozenColumns : [ [{
				width : '5%',
				title : shipmentMonth,
				field : 'shipmentWeek',
			...
			/* 	$(".datagrid-header-row td div span").each(function(i,th){
	            ...
	            var shipmentNos = 0//统计当月出货总数
	            ,shipmentTotals=0//统计累计出货总数
	            ,quantitys=0;  //统计订单总数
	            var shipmentNoss = 0//统计当月出货总数
	            ,shipmentTotalss=0//统计累计出货总数
	            ,quantityss=0;  //统计订单总数
	            var week = "第1周";
	            var tempWeek;
	            var i = 0;
	            for (i; i < rows.length; i++) {//遍历所有行
	            	if (i == 0) {
	            		week = rows[i]['shipmentWeek'];
	            	}
	            	tempWeek = rows[i]['shipmentWeek'];
	            	if (week != tempWeek) {
	            		week = rows[i]['shipmentWeek'];
	            		dataGrid.datagrid('insertRow',{//追加行
	            			index: i,	// 索引从0开始
	            			row: {
	            				shipmentWeek: rows[(i-1)]['shipmentWeek'],
	            				productType: '合计',
								quantity: quantitys,
								shipmentNo: shipmentNos,
								shipmentTotal: shipmentTotals,
								none : "noneFlag",
	            			}
						});
	            		quantitys = 0;shipmentTotals=0;shipmentNos = 0;
	            	} else {
		            	quantitys += rows[i]['quantity'];
		            	quantityss += rows[i]['quantity'];
						shipmentNos += rows[i]['shipmentNo'];
						shipmentNoss += rows[i]['shipmentNo'];
		            	shipmentTotals += rows[i]['shipmentTotal'];
		            	shipmentTotalss += rows[i]['shipmentTotal'];
	            	}
	            }
	            if (i > 0) {
		           //添加最后一行统计
	        	   dataGrid.datagrid('appendRow',{
	            	...
	            	dataGrid.datagrid('appendRow',{
						productType: '总计',
						quantity: quantityss,
						shipmentNo: shipmentNoss,
						shipmentTotal: shipmentTotalss,
						none : "noneFlag",
					});
	            }
	            //所有列进行合并操作
		        ...
		        //指定列进行合并操作
		        $(this).datagrid("autoMergeCells", ['shipmentWeek']);
	...
	function currMonStatFun() {
		var defaultDate = new Date();
		var shipmentMonthCurrent = defaultDate.getFullYear() + '-' + (defaultDate.getMonth() + 1);
		var url = '${ctx}' + '/saleOut/getMonthPage?shipmentMonth='+shipmentMonthCurrent;
	  	var title = "出库汇总:"+shipmentMonthCurrent;
		window.parent.addTab({
  			id : shipmentMonthCurrent, // 给iframe赋予一个id，方便从新打开判断是否是相同的单
			url : url,
			title : title,
		});
	}
