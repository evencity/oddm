		        	...
		        	$.ajax({
		        		...
		        		success : function(result) {
		        			if (result.success) {
		    	                ...
		    	                if (result.obj == "${salePiDto.id}") { //如果是自己的单
			        				isNotExist = true;
		    	                } else {
			    	                isNotExist = false;
		    	                }
...
<div class="styleeasyui-layout" data-options="fit:true,border:false" >
	<div data-options="region:'center',border:false" title="" style="overflow: auto;padding: 0px;width: 100%;" >
		<form id="salePiEditForm" method="post">
		<!-- 基础信息 -->
	    <table border="0" cellpadding="0" cellspacing="14px" class="ordertable">
	        ...
	        <tr>
	            ...
	            <td class=""> <input id="piNo" value="${salePiDto.piNo }" class="easyui-validatebox" data-options="required:true,validType:['piNoValidate','piNoValidatex']"/></td>
	            <td class="lefttd" width="15%">客户名称：</td>
	            <td class=""> <input id="clientName" value="${salePiDto.clientName }" class="easyui-validatebox" data-options="required:true,validType:['length[0,64]']"/></td>
	           	<td class="lefttd" width="10%">业务员：</td>
	            <td class=""> <input id="seller" value="${salePiDto.seller }" class="" data-options="required:true,validType:['length[0,64]']"/></td>
	        	<td class="lefttd" width="10%">国家：</td>
	            <td class=""> <input id="district" value="${salePiDto.district }" class="" data-options="required:true,validType:['length[0,64]']"/></td>
	        </tr>
     	    <tr>
	            <td class="lefttd" width="15%">摘要：</td>
	            <td class=""><input id="summary" value="${salePiDto.summary }" class="easyui-validatebox" data-options="required:true,validType:['length[0,64]']"/></td>
	            <td class="lefttd" width="15%">下单日期：</td>
	            <td class=""><input id="orderDate" value="${salePiDto.orderDate }" class="easyui-datebox"  data-options="editable:false,required:true" style="height: 30px"/></td> 
	            <td class="lefttd" width="10%">总金额：</td>
	            <td class=""><input id="amount" value="${salePiDto.amount }"/></td>
	            <td class="lefttd" width="10%">币种：</td>
	            <td class=""><input id="currency" value="${salePiDto.currency }" name="currency" style="width: 52%;color:red" type="text" placeholder="输入币种" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']"/></td>
	        </tr>
	        <tr>
	            <td class="lefttd" width="15%">PO编号：</td>
	            <td class="" colspan="3"><textarea id="poNo" class="textareacss easyui-validatebox" data-options="required:false,validType:['poNoValidate','length[0,200]']" rows="3" onkeydown="checkEnter(event)">${salePiDto.poNo }</textarea></td>
	            <td class="lefttd" width="15%">备注：</td>
	            <td class="" colspan="3"><textarea id="description" class="textareacss" rows="3" maxlength="200">${salePiDto.description }</textarea></td>
	        </tr>
	    </table>
...
function summit() {
	var isValid = $('#salePiEditForm').form('validate'); // 验证不通过则不执行
	if (!isValid) {
		return; // 验证不通过则不执行
	}
