	    ...
	    <table border="0" cellpadding="0" cellspacing="0px" class="ordertable" style="padding-bottom:10px;">
    	  <caption  class="tabletitle">样机明细：</caption >
     	  <tr>
	            ...
	            <td class="td_body"> <input id="clientName" value="${saleProtoDto.clientName }" class="easyui-validatebox" data-options="required:true,validType:['length[0,64]']"/></td>
           	    <td class="lefttd" width="10%">国家区域：</td>
	            <td class="td_body"> <input id="district" value="${saleProtoDto.district }" class="easyui-validatebox" data-options="required:true,validType:['length[0,100]']"/></td>
	            <td class="lefttd" width="10%">产品名称：</td>
	            <td class="td_body"> <input id="productName" value="${saleProtoDto.productName }" class="easyui-validatebox" data-options="required:true,validType:['length[0,64]']"/></td>
	            <td class="lefttd" width="10%">尺寸：</td>
	            <td class="td_body"> <input id="size" value="${saleProtoDto.size }"/></td>
	        </tr>
	        <tr>
	       		<td class="lefttd" width="10%">外壳型号：</td>
	            <td class="td_body"> <input id="shell" value="${saleProtoDto.shell }" class="easyui-validatebox" data-options="required:false,validType:['length[0,64]']"/></td>
	            <td class="lefttd" width="10%">外观/软件：</td>
	            <td class="td_body"> <input id="facade" value="${saleProtoDto.facade }" class="easyui-validatebox" data-options="required:true,validType:['length[0,64]']"/></td>
	        	<td class="lefttd" width="10%">数量：</td>
	            <td class="td_body"> <input id="quantity" value="${saleProtoDto.quantity }"/></td>
          	    <td class="lefttd" width="10%">是否收款：</td>
	            <td class="td_body"> <input id="payed" value="${saleProtoDto.payed }" class="easyui-validatebox" data-options="required:false,validType:['length[0,64]']"/></td>
	             </tr>
	        <tr>
	            <td class="lefttd" width="10%">送样日期：</td>
	            <td class="td_body"><input id="dateSend" value="${saleProtoDto.dateSend }" class="easyui-datebox"  data-options="editable:false,required:true" style="height: 30px"/></td> 
	         	<td class="lefttd" width="10%">收款日期：</td>
	            <td class="td_body"><input id="datePayed" value="${saleProtoDto.datePayed }" class="easyui-datebox"  data-options="editable:false,required:false" style="height: 30px"/></td> 
	           	<td class="lefttd" width="10%">退还日期：</td>
	            <td class="td_body"> <input id="dateReturn" value="${saleProtoDto.dateReturn }" class="easyui-datebox"  data-options="editable:false,required:false" style="height: 30px"/></td>
	           	<td class="lefttd" width="10%"></td>
	            <td class="td_body"></td>
	           </tr>
	          	<td class="lefttd" width="10%">PCBA型号：</td>
	            <td class="td_body" colspan="3"> <textarea id="pcba"  class="textareacss" rows="3" maxlength="100" onkeydown="checkEnter(event)">${saleProtoDto.pcba }</textarea></td>
	            <td class="lefttd" width="10%">备注：</td>
	            <td class="td_body" colspan="3" ><textarea id="description" class="textareacss" rows="3" maxlength="200">${saleProtoDto.description }</textarea></td>
	        </tr>
	    </table>
...
function summit() {
	var isValid = $('#saleProtoEditForm').form('validate'); // 验证不通过则不执行
	if (!isValid) {
		return; // 验证不通过则不执行
	}
