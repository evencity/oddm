...
<script type="text/javascript">	
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					$.ajax({
						type: 'POST',
					    url: "${ctx}/saleInfo/checkOrderForOrder?orderNo="+orderNo, 
					    dataType: "json",
	...
	function searchFun() {
		...
		if(orderNo == "" && clientName == "" && seller == "" && dateOrderStart == "" && dateOrderEnd == ""){
		    ...
		    buttons:[{
				...
				handler:function(){
		 		...
		 		/* 	if (clientName == "" && seller == "" && productType == "") {
		 			...
		 			download_file("${ctx}/saleInfo/excelExport?clientName="+clientName+"&seller="+seller
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/saleInfo/addAll')}">
			<a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
		</c:if>
		<c:if test="${fn:contains(sessionInfo.resourceList, '/saleInfo/editAll')}">
			<a onclick="editFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-edit'">编辑</a>
		</c:if> 
		<c:if test="${fn:contains(sessionInfo.resourceList, '/saleInfo/deleteAll')}">
			<a onclick="deleteFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-del'">删除</a>
		</c:if> 
		 <c:if test="${fn:contains(sessionInfo.resourceList, '/saleInfo/excelAll')}">
