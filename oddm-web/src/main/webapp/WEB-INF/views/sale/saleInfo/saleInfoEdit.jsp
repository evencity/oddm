	...
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;padding: 0px;width: 100%;" >
		...
		<table border="0" cellpadding="0" cellspacing="0" class="ordertable">
	        ...
	        <tr>
	            ...
	            <td class="righttd" id="td_quantity" width="15%"><div id="quantity" style="color: red;">${saleInfoDto.quantity}</div> </td>
	        </tr>
	        <tr>
	    ...
	    <table border="0" cellpadding="0" cellspacing="0" class="ordertable">
	        <tr class="tabletitle">
	         ...
	         <tr>
	            ...
	            <td class="" width="15%"> <input id="quantitySale" value="${saleInfoDto.quantitySale}"/></td>
	            <td class="lefttd" width="10%">单价：
	            <td class="" width="15%"><input id="unitPrice" value="${saleInfoDto.unitPrice}"/></td>
	        	<td class="lefttd" width="10%">总金额：</td>
	            <td class="righttd" id="total" width="15%">0.00</td>
	            <td class="lefttd" width="10%">币种：</td>
	            <td class="" width="15%"><input id="currency" name="currency" style="width: 52%;height: 100%;color:red" type="text" placeholder="输入币种" class="easyui-validatebox" data-options="required:true,validType:['length[0,20]']" value="${saleInfoDto.currency}"/></td>
	        </tr>
	         <tr>
	            <td class="lefttd" width="10%">备品数量：</td>
	            <td class="righttd" width="15%"><input id="spare" value="${saleInfoDto.spare}"/></td>
	        </tr>
	        <tr>
	            <td class="lefttd">型号规格：</td>
	            <td class="righttd" colspan="3"><textarea id="specification"  class="textareacss" rows="3" maxlength="200">${saleInfoDto.specification}</textarea></td>
	            <td class="lefttd" >备注：</td>
	            <td class="righttd" colspan="3"><textarea id="description"  class="textareacss" rows="3" maxlength="200">${saleInfoDto.description}</textarea></td>
	        </tr>
	    </table>
...
function summit() {
	var isValid = $('#saleInfoEditForm').form('validate'); // 验证不通过则不执行
	if (!isValid) {
		return; // 验证不通过则不执行
	}
	...
	$.ajax({
		url : '${pageContext.request.contextPath}/saleInfo/edit',
		type : "POST",
