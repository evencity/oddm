...
<style>
	...
	.bordertable tr td{ border:1px solid #ccc; padding:5px 0 5px 5px;height:20px;word-wrap: break-word}
	...
	.showContactCss{cursor: pointer;color: #06c;text-decoration: underline;word-wrap:break-word}
	.choose{color: #06c;cursor: pointer;}
...
<script type="text/javascript">
	...
	function bomReviewFun(){
		parent.$.messager.confirm('询问', '您是否确定审核通过？(操作后将会产生<span style="color:#f60">变更记录</span>！)', function(b) {
			if (b) {
				progressLoad();
				var orderNo = $('#orderNo').html() == null ? '' : $('#orderNo').html().trim();
				$.post('${ctx}/bom/review', {
					id : bomId,
					orderNo : orderNo
				}, function(result) {
