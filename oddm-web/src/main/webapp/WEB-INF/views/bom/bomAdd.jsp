...
<style>
	...
	.tabletop th{text-align: center;padding:3px 0 3px 0px;}
	.inputs{border:1px solid #CCC;width:95%;padding:5px 0 2px 4px;font-size:12px;color:#069;border-radius: 4px;}
	...
	.textareacss{width:98%;border:1px solid #CCC;text-align: left;font-size:12px;color:#069;border-radius: 4px;height: 40px}
...
<script type="text/javascript">
	...
	//物料中materialCode判断当前bom页面有没有重复，materialCodeAuto
	$(function() {
	/* 	$.ajax({
 ...
 */
			...
			buttons : [ {
				...
				handler : function() {
					if(parent.$('#chooseMaterialMain li').length > 0){
						...
						$('#'+tableId).find('.contain'+tableId).each(function(){
							$(this).remove();
						})
	...
	function addBomFun(){
		...
		var isValid = $('#addBomForm').form('validate');
...
<div id='content'>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <tr>
            ...
            <td class="righttd" width="15%">${orderInfoDTO.orderNo}</td>
            <td class="lefttd" width="10%">工厂机型:</td>
            <td class="righttd" width="15%">${orderInfoDTO.productFactory}</td>
            <td class="lefttd" width="10%">数量:</td>
            <td class="righttd" width="15%">${orderInfoDTO.quantity}</td>
            <td class="lefttd" width="10%">所属业务:</td>
            <td class="righttd" width="15%">${orderInfoDTO.seller}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="righttd"><input type="text" class="inputs easyui-validatebox materialCode"  id="materialCode" data-options="required:true,validType:['isBlank_','materialCodeLengthValidate[14]','materialCodeValidateMain','materialCodeValidate']"/></td>
        ...
        <tr>
            ...
            <td class="righttd" colspan="3"><textarea id="specification" class="textareacss easyui-validatebox" data-options="validType:['isBlank_','length[0,200]']"></textarea></td>
            <td class="lefttd">描述:</td>
            <td class="righttd" colspan="3"><textarea id="description" class="textareacss easyui-validatebox" data-options="validType:['isBlank_','length[0,200]']"></textarea></td>
        </tr>
    </table>
	   ...
	   <!-- 一、裸机成品 -->
	    <table id="tableTitle_2" border="0" cellpadding="0" cellspacing="0" class="ordertable" style="margin-bottom: 0px;margin-top: 0px">
   	    ...
   	    <!-- <table id="tableContent_2" border="0" cellpadding="0" cellspacing="0" class="bordertable">
		     <tr class="tabletop">
		        <th width="15%" class="">物料编码</th>
		        <th width="20%" class="">品名</th>
		        <th width="25%" class="">规格型号及封装</th>
		        <th width="10%" class="">品牌</th>
		        <th width="5%" class="">用量</th>
		        <th width="17%" class="">描述</th>
		        <th width="8%" class="">联系人</th>
		        <th align="right" style="border-left: 0px">
					<span onclick="addbomMaterialFun('tableContent_2');" class="icon-add tip"  title="添加"></span>
				</th>
		    </tr>
	    </table> -->
...
<div align="center">
	<button id="" class='btn-default' onclick="addBomFun()">提交</button> 
</div>
</body>