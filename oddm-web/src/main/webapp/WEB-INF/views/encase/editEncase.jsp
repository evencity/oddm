...
<div class="">
    <table border="0px" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <tr class="f1">
         	 <td style="display:none"><input class="inputs" name="idz" value="${encaseInfo.id}"></td>
             <td class="lefttd">装箱单名称：</td>
             <td class="righttd" ><input class="inputs" name="namez" value="${encaseInfo.name}"></td>
			 <td class="lefttd">装箱日期：</td>
     		 <td class="righttd"><input class="inputs easyui-datebox form_datetime"  value="${encaseInfo.encaseDate}" id="encaseDate" size="16"></td>
     		 <td class="lefttd">创建日期：</td>
     		 <td class="righttd"><input class="inputs easyui-datebox form_datetime"  value="${encaseInfo.timestamp}" id="timestamp" size="16"></td>
        </tr>
        <tr class="f1">
            <td class="lefttd">公司名称：</td>
            <td class="righttd"><input class="inputs" name="company" value="${encaseInfo.company}"></td>
           	<td class="lefttd">Zipcode：</td>
            <td class="righttd"><input class="inputs" name="zipcode" value="${encaseInfo.zipcode}"></td>
            <td class="lefttd">Tel：</td>
            <td class="righttd"><input class="inputs" name="telphone" value="${encaseInfo.telphone}"></td>
            </td>
        </tr>
        <tr class="f1">
        	<td class="lefttd">Add：</td>
            <td class="righttd" colspan="3"><input class="inputs" style="width:540px" name="address" value="${encaseInfo.address}"> </td>
            <td class="lefttd">重量单位：</td>
	         ...
	         msg="g|kg|t" missingMessage="此项为必填项" required="true" value="${encaseInfo.unitWeight}"
	         /></td>
        </tr>
        <tr class="f1">
         	<td class="lefttd">Homepage：</td>
            <td class="righttd" colspan="3"><input style="width:540px;" class="inputs" name="homepage" value="${encaseInfo.homepage}"/></td>
	        ...
	        msg="M|CM|MM" missingMessage="此项为必填项" required="true" value="${encaseInfo.unitLength}"
	        /></td>
        </tr>
    </table>
    <table border="1px" cellpadding="0" cellspacing="0" class="bordertable" id="declare_customs_table">
        ...
        <tr style="background: #DCE0E2;font-weight: 800;">
        	...
        	<td colspan="2" width="7%">Item No.<br>产品编号(客户型号)</td>
        	<td colspan="1" width="5%">UNIT<br>单位</td>
        ...
        <tbody class="f22">
         <c:forEach items="${encaseInfo.encaseList}" var="item">
	         <tr class="declare_customs_addBefore f2">
	            <td style="display:none"><input style="width:130px;" class="inputz" name="id" value="${item['id']}"></td>
	            <td colspan="2"><input style="width:95%" class="inputz" name="CNo" value="${item['CNo']}"></td>
	            <td colspan="2"><input style="width:95%" class="inputz" name="itemNo" value="${item['itemNo']}"></td>
	            <td colspan="1"><input style="width:95%" class="inputz" name="description" value="${item['description']}"></td>
	            <td colspan="2"><input style="width:95%" class="inputz unit" name="unit" value="${item['unit']}"></td>
	            <td colspan="1"><input style="width:95%" class="inputz" name="remark" value="${item['remark']}"></td>
	            ...
	            <td colspan="1" width="1%"><img class="dels" style="" id="${item['id']}" alt="" src="${pageContext.request.contextPath}/lib/images/del.jpg"></td>
	        </tr>
	        </c:forEach>
        </tbody>
        ...
        <tr style="background: #DCE0E2;font-weight: 800;">
           <!--  <td rowspan="2">QTY/Ctn<br>每箱的数量</td> -->
            <td >N.W./n<br>净重</td>
            <td >G.W./n<br>毛重</td>
            <td colspan="3" width="10%">Carton Size(MM)/n<br>单个纸箱尺寸</td>
        ...
        <tr style="background: #DCE0E2;font-weight: 800;">
 		...
 		<tbody class="f24">
 		 <c:forEach items="${encaseInfo.encaseList}" var="item">
	        <tr class="declare_customs_addBefore f2"  id="${item['id']}">
	            <td ><input class="inputz NW" style="width:95%" name="nW" value="${item['nW']}"></td>
	            <td ><input class="inputz GW" style="width:95%" name="gW" value="${item['gW']}"></td>
	            <td ><input class="inputz M1" style="width:95%" name="length" value="${item['length']}"></td>
	            <td ><input class="inputz M2" style="width:95%" name="width" value="${item['width']}"></td>
	            <td ><input class="inputz M3" style="width:95%" name="hight" value="${item['hight']}"></td>
	            <td ><input class="inputz qtyCtn" style="width:95%" name="qtyCtn" value="${item['qtyCtn']}"></td>
	         	<td ><input class="inputz qty" style="width:95%" name="qty" value="${item['qty']}"></td>
       ...
       <!-- 	<tbody class="f23">
        ...
        <tr class="f1">
            ...
            <td colspan="10"><textarea style="width:98%;height:100px;" class="inputs" name="description">${encaseInfo.description}</textarea></td>
        </tr>
        ...
        <tr>
            ...
            <td style="height: 40px" onmouseover="nwz($(this));"><span></span></td>
            <td style="height: 40px" colspan="2" onmouseover="gwz($(this));"><span></span></td>
        </tr>
...
<div align="center">
    <button id="editEncase" class="btn34">保存</button>
</div>
<script>
$(function(){
	var unitLength = $(".inputs[name=unitLength]").val();
	var unitWeight = $(".inputs[name=unitWeight]").val();
	$(".unitLength").text("("+unitLength+"³)");
	$(".unitWight").text("("+unitWeight+")");
})
$("#editEncase").on("click",function(){
	...
	//	  l += name +":"+ $this.val()+",";
		 ...
		 c.find(".inputz[name=cNo]").each(function(){
			 	var id = c.find('.inputz[name=id]').val();
		...
		params = {
	    	 ...
	    	 $.ajax({
				type: "POST",
				url: "${pageContext.request.contextPath}/encase/edit",
...
$(".dels").on("click",function(){
	...
	var id = $(this).attr("id");
	 $.ajax({
			type: "POST",
			url: "${pageContext.request.contextPath}/encase/delEncaseList?id="+id,
			dataType:"json",
			async: false,
			contentType: "application/json; charset=utf-8",
			success: function(j) {
...
$("#adds").on("click",function(){
	...
	$(".f22").append( '<tr class="declare_customs_addBefore f2 '+flag+'" >'
			+'<td style="display:none"><input style="width:130px;" class="inputz" name="id"></td>'
