...
<style>
	...
	.tabletop th{padding:4px 0 4px 0px;text-align: center;}
...
<script type="text/javascript">
		...
		$.ajax({
            ...
            success: function(data){
            	if(data != null){
            		...
            		currencyChar = toCharFun(data.currency);//加载默认货币字符
            		$('#UNITPRICE_title').text("UNITPRICE("+data.currency+")")
					$('#AMOUNT_title').text("AMOUNT("+data.currency+")")
            		...
            		$('#dateInvoice').datebox('setValue',data.dateInvoiceString)
            		...
            		if(data.invoiceListDTOs != null && data.invoiceListDTOs.length > 0){
            			...
            			for(var i in invoiceLists){
            				...
            				var $content = $('<tr class="invoiceList">'+
            						'<td style="padding:1px"><textarea class="textareacss description">'+description+'</textarea></td>'+
            						'<td style="padding:1px"><input class="qty" style="width:98%" value="'+qty+'"></td>'+
            						'<td style="padding:1px"><input class="unitPrice " style="width:98%"  value="'+unitPrice+'"></td>'+
            						'<td style="padding:1px" ><span class="currencyChar">'+currencyChar+'</span><span class="total">'+total+'</span></td>'+
            						'<td style="padding:1px"><input class="orderNo inputs"  value="'+orderNo+'"></td>'+
            						'<td style="padding:1px"><input class="model inputs"  value="'+model+'"></td>'+
            						'<td style="padding: 0px">'+
            							'<span onclick="delInvoiceListFun(this);" class="icon-del tip"  title="删除"></span>'+
            						'</td>'+
            					'</tr>');
            				$content.data('id',invoiceLists[i].id)
	...
	function editInvoiceFun(){
		var isValid = $('#editInvoiceForm').form('validate');
		console.log("editInvoiceForm 验证"+isValid)
		if(!isValid){
			return;
		}
		...
		$('#invoiceListTable').find('.invoiceList').each(function(){
			var $trValue = $(this);
			var invoiceList = {
					id : typeof($(this).data('id')) == 'undefined' ? null : $(this).data('id'),
...
<!-- <div id='content'>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <tr>
            ...
            <td class="righttd" width="15%" ><input type="text" class="inputs "  id="name" /></td>
...
<div align="center">
	<button id="" class='btn-default' onclick="editInvoiceFun()">提交</button> 
</div>
</body>