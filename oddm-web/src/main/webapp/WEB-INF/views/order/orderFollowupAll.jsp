...
<title>跟单信息</title>
	...
	$(function() {
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}/orderFollowup/dataGridAll',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					$.ajax({
						type: 'POST',
					    url: "${ctx}/orderFollowup/checkOrderForOrderAll?orderNo="+orderNo, 
					    dataType: "json",
					    ...
					    success: function(data){
					    	if(data.success){
					    		...
					    		parent.$.messager.alert('提示', '订单号不存在！', 'info');
					    	}
					    }
					})
				}
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					download_file("${ctx}/orderFollowup/excelExportAll?state="+orderFollowExcel+"&seller="+seller+"&clientName="+clientName+"&merchandiser="+merchandiser+"&dateOrderStart="+dateOrderStart+"&dateOrderEnd="+dateOrderEnd);
					parent.$.modalDialog.handler.dialog('close');
				}
			} ]
		});
...
<body class="easyui-layout" data-options="fit:true,border:false">
	 <div data-options="region:'north',border:false" style="height: 60px; overflow: hidden;background-color: #fff">
		<!-- <form id="searchForm"> -->
			<table>
				...
				<tr>
					<th>业务员</th>
					<td><input name="seller" placeholder="请输入业务员名称" id="seller"/></td>
					<th>跟单员</th>
	...
	<div data-options="region:'center',border:false" style="60px">
		<table id="dataGrid" data-options="fit:true,border:false"></table>
	</div> 
	<div id="toolbar" style="display: none;">
		
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderFollowup/allAdd')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderFollowup/allEdit')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderFollowup/allDelete')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderFollowup/allExcel')}">
