...
<style>
	...
	.inputs:focus{border:1px solid #f60;width:90%;padding:5px 0 2px 4px;font-size:12px;color:#999;}
	.lefttd{width:85px;}
...
<script type="text/javascript">
	var orderFollowupId = "${orderFollowupId}";
	var state = 1;
	$(function() {
		 $.ajax({
			url:"${ctx}/orderFollowup/get?id="+ orderFollowupId,
			type:"GET",
			dataType: "json",
            contentType: "application/json; charset=utf-8", 
            success: function(data){
            	/* if(data.state == 1){
            		$('#submitOrderFollowupBtn').css('display','inline-block')
            	} */
            	console.log(data)
            	if(data.state == 2){
            		$('#finishOrderFollowupBtn').css('display','inline-block')
            	}
            	if(data.state == 3){
            		$('#editOrderFollowupBtn').css('display','inline-block')
            	}
            	if(data.state != null){
            		state = data.state;
            	}
            	...
            	$('#orderNo').data('orderId',data.orderInfoDTO.id)
            	...
            	$('#dateClient').datebox('setValue',data.dateClient);
            	$('#dateFactory').datebox('setValue',data.dateFactory);
            	console.log($('#dateClient'))
            	//$('#dateShipment').datebox('setValue',data.dateShipment);
            	
            	$("input[name=bootLogo][value="+data.bootLogo+"]").attr("checked",true);
            	$("input[name=uuid][value="+data.uuid+"]").attr("checked",true);
            	$("input[name=specification][value="+data.specification+"]").attr("checked",true);
            	$("input[name=carton][value="+data.carton+"]").attr("checked",true);
            	$("input[name=map][value="+data.map+"]").attr("checked",true);
            	$("input[name=shell][value="+data.shell+"]").attr("checked",true);
            	$("input[name=membrane][value="+data.membrane+"]").attr("checked",true);
            	$("input[name=fitting][value="+data.fitting+"]").attr("checked",true);
            	$("input[name=sorfware][value="+data.sorfware+"]").attr("checked",true);
            	$("input[name=hardware][value="+data.hardware+"]").attr("checked",true);
            	$("input[name=colorbox][value="+data.colorbox+"]").attr("checked",true);
            	$("input[name=inspection][value="+data.inspection+"]").attr("checked",true);
            	$("input[name=preFile][value="+data.preFile+"]").attr("checked",true);
            	$("input[name=tags][value="+data.tags+"]").attr("checked",true);
            	$("input[name=packing][value="+data.packing+"]").attr("checked",true);
            	$("input[name=agency][value="+data.agency+"]").attr("checked",true);
            	$("input[name=payment][value="+data.payment+"]").attr("checked",true);
            	
            	$('#dateFactory').val(data.dateFactory);
				$('#dateShipment').val(data.dateShipment)
				...
				if(data.orderFollowupOutDTOs != null && data.orderFollowupOutDTOs.length > 0){
					for(var i in data.orderFollowupOutDTOs){
						var shipmentDate = data.orderFollowupOutDTOs[i].shipmentDate == null ? null :data.orderFollowupOutDTOs[i].shipmentDate;
    					var shipmentNo = data.orderFollowupOutDTOs[i].shipmentNo == null ? null :data.orderFollowupOutDTOs[i].shipmentNo;
    					var $content = $('<tr class="orderFollowupOut">'+
    							...
    							'</td>'+
    				        '</tr>');
    					$content.data('id',data.orderFollowupOutDTOs[i].id )
    					...
    					$content.find('.shipmentDate').datebox('setValue',shipmentDate);
    					$content.find('.shipmentNo').numberbox('setValue',shipmentNo);
    					total += shipmentNo;
					}
		...
		$('#editOrderFollowupBtn').off('click').on('click',function(){
        	...
        	var shipmentNo = $('#shipmentNo').numberbox('getValue');
        	if(parseInt(shipmentNo) > 100000000){
				parent.$.messager.alert('提示', '最近出货数量不能超过100000000！', 'info');
		        return false;
			}
        	if(parseInt(shipmentNo) < 1){
				parent.$.messager.alert('提示', '最近出货数量不能小于1！', 'info');
		        return false;
			}
			var shipmentTotal = $('#shipmentTotal').html() */
		...
		$('#finishOrderFollowupBtn').off('click').on('click',function(){
			var isValid = $('#orderFollowupForm').form('validate');
			console.log("orderFollowupForm 验证"+isValid)
			if(!isValid){
				return;
        	...
        	var plan = $('#plan').val();
        	 if(plan.length < 1){
				parent.$.messager.alert('提示', '方案不能为空！', 'info');
		        return false;
			} 
        	 if(plan.length > 64){
 				parent.$.messager.alert('提示', '方案不能超过64个字符！', 'info');
        	...
        	var dateClient = $('#dateClient').datebox('getValue');
        	 if(dateClient.length < 1){
				parent.$.messager.alert('提示', '客户交期不能为空！', 'info');
		        return false;
			} 
        	var dateFactory = $('#dateFactory').datebox('getValue');
        	 if(dateFactory.length < 1){
				parent.$.messager.alert('提示', '计划回复交期不能为空！', 'info');
		        return false;
			} 
        	/* var dateShipment = $('#dateShipment').datebox('getValue');
        	 if(dateShipment.length < 1){
				parent.$.messager.alert('提示', '最近出货日期不能为空！', 'info');
		        return false;
			} 
        	var shipmentNo = $('#shipmentNo').numberbox('getValue');
        	if(shipmentNo.length < 1){
				parent.$.messager.alert('提示', '最近出货数量不能为空！', 'info');
		        return false;
			} 
        	if(parseInt(shipmentNo) > 100000000){
				parent.$.messager.alert('提示', '最近出货数量不能超过100000000！', 'info');
			...
			var status = $('#status').val()
			if(status.length < 1){
				parent.$.messager.alert('提示', '更新状态不能为空！', 'info');
		        return false;
			} 
			if(status.length > 200){
				parent.$.messager.alert('提示', '更新状态不能超过200个字符！', 'info');
			...
			if(description.length < 1){
				parent.$.messager.alert('提示', '备注不能为空！', 'info');
		        return false;
			} 
			if(description.length > 200){
				parent.$.messager.alert('提示', '备注不能超过200个字符！', 'info');
          ...
          /*  	var orderFollowupOuts = [];
			...
			$.ajax({
	         	type: 'POST',
	             url: "${ctx}/orderFollowup/editForFinish" ,
	             data: JSON.stringify(param) ,
	             dataType: "json",
	             contentType: "application/json; charset=utf-8",
	             async: false,
	...
	function deleteOrderFollowupOutFun(obj){
		...
		var shipmentDate = tdValue.eq(1).find('input').datebox('getValue');
   		var shipmentNo =  tdValue.eq(3).find('input').numberbox('getValue');
   		console.log(shipmentDate != '' || shipmentNo != '' )
		if(shipmentDate != '' || shipmentNo != '' ){
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <tr>
            ...
            <td class="righttd" colspan="3">
            	<input type="radio" value="" name="level">无
...
<div  align="center">
	<button class="btn-default" id="editOrderFollowupBtn">修改跟单</button>
	<!-- <button class="btn-default hiddenBtn" id="submitOrderFollowupBtn">正式提交</button> -->
	<button class="btn-default hiddenBtn" id="finishOrderFollowupBtn">跟单完结</button>
</div>