...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#addNewOrder').off('click').on('click',function(){
			//基本信息
			var orderNo = $('#orderNo').val().trim();
			 if(!/^[a-zA-Z0-9]{10,10}$/.test(orderNo)){
			...
			if(productFactory.length < 1){
				parent.$.messager.alert('提示', '工厂机型不能为空！', 'info');
				 $('#productFactory').focus();
				$('#productFactory').select(); 
		        return false;
			}
			...
			if(productClient.length < 1){
				alert(productClient.length)
				parent.$.messager.alert('提示', '客户机型不能为空！', 'info');
				 $('#productClient').focus();
				$('#productClient').select(); 
		        return false;
			}
			...
			var isrepeat = Number($("input[name=isrepeat]:checked").val());
			...
			$('#materialBare_shell').parent().find('.contain').each(function(index,element){
				var tdValue = $(this).children();
				var name = tdValue.eq(0).find('span').text().trim().trim();
		...
		$.ajax({
			url:"${ctx}/order/getForAddNew?id="+ pid,
			type:"GET",
			dataType: "json",
            contentType: "application/json; charset=utf-8", 
            success: function(data){
            	$('#newOrderMsg').html('翻单号：'+data.orderNo+"_"+'客户型号：'+data.productClient);
...
<div>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <tr class="">
            <td class="lefttd">翻单信息：</td>
            <td class="righttd" colspan="7"><div id="newOrderMsg"></div> </td>
        </tr>
        <tr>
            ...
            <td class="righttd"><input class="inputs" id="bizName"> </td> -->
            <td class="lefttd"> 工厂机型：
            <td class="righttd"><input type="text" class="inputs easyui-validatebox"  id="productFactory" missingMessage="此项为必填项" required="true" validType="productFactoryValidate"/> </td>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="ordertable">
        ...
        <td class="lefttd">软件：</td>
            <td class="righttd">
            	<div  class="color_f60" id="isrepeat">
            		<label><input type="radio" name="isrepeat" value="1" />新软件</label>
              		<label><input type="radio" name="isrepeat" value="2" checked="checked"/>翻单软件</label>
            	</div>
            </td>
    ...
    <!-- 配件信息-->
     ...
     <table border="0" cellpadding="0" cellspacing="0" class="bordertable">
        ...
        <tr>
        	<td colspan="3"><textarea id="orderSpare" class="textareacss"  cols="1" rows="8"> </textarea></td>
        </tr>
    </table>
     <!-- 备品说明-->
     <table border="0" cellpadding="0" cellspacing="0"  class="bordertable">
        ...
        <tr>
        	<td colspan="3"><textarea  id="description" class="textareacss"  cols="1" rows="8"> </textarea></td>
        </tr>
	</table>
	</div>
	<div class=""  align="center">
		<a id="addNewOrder" class="btn-default">创建翻单</a>
	</div>
</body>