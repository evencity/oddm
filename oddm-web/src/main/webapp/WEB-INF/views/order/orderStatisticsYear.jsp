...
<title>订单信息</title>
	...
	$(function() {
        ...
        $('#category, #dateYear').parent().children().css({
       		"width":"100%",
        });
        $('#category, #dateYear').next().find('input').css({
        	"width":"100%",
        });
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}/order/dataGridStatisticsYear?year='+currentYear+'&type=1',
			striped : true,
			pagination : true,
			singleSelect : true,
			idField : 'id',
	...
	function searchFun() {
		...
		if(category == "" && year == ""){
			...
			dataGrid.datagrid({url : '${ctx}/order/dataGridStatisticsYear?year='+year+'&type='+type
			});  
		}  
	}
