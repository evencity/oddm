...
<title>订单信息</title>
	...
	$(function() {
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}/document/dataOrderInfoGridAll',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
		...
		$("body").keydown(function(event) {
		    if (event.keyCode == "13") {//keyCode=13是回车键
				...
				if(orderNo == "" && clientName == "" && productClient == "" && seller == "" && productFactory == "" && merchandiser == ""){
	 ...
	 <div data-options="region:'north',border:false" style="height: 60px; overflow: hidden;background-color: #fff">
		<!-- <form id="searchForm"> -->
			<table>
				...
				<tr>
					<th>所属业务:</th>
					<td><input name="seller" placeholder="请输入业务名称" id="seller" /></td> 
					<th>所属跟单:</th>
					<td><input name="merchandiser" placeholder="请输入所属跟单" id="merchandiser" /></td>
				</tr>
			</table>
		<!-- </form> -->
	</div> 
